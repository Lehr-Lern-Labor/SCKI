...
def _():
    ...
    TRAIN_DATA_PATH = 'public/Files/SC-KI_2022/train'
    TEST_DATA_PATH = 'public/Files/SC-KI_2022/test'
    return (
        DataLoader,
        TEST_DATA_PATH,
        TRAIN_DATA_PATH,
...
def _(torch):
    def test_model(model, data):
        ...
        for (_x, _y) in data:
            _output = model(_x)
            _output = torch.argmax(_output, dim=1)
            correct = correct + sum(torch.eq(_output, _y)).item()
            total = total + len(_y)
...
def _(CNN, torch):
    ...
    return LERNRATE, cnn, loss_func, optimizer
...
def _(cnn, plt, test_loader, torch):
    def show(x, y, i):
        print('Klasse', _output[i].item(), '/', _y[i].item(), '-', correct[i].item())
    ...
    for (_x, _y) in test_loader:
        ...
        for i in range(0, len(correct)):
            ...
            if incorr >= 10:
                break
    return corr, correct, i, incorr, show
