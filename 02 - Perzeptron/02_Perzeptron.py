...
def _(mo):
    mo.md(
        ...
        ---
        ✎ Setze das Perzeptron als Funktion in Python um. Rufe deine Funktion mit von dir gewählten Werten für $w_1$, $w_2$ und $b$ auf und überprüfe rechnerisch, ob die Ausgabe stimmt.

        🗬 *Verwende als Aktivierungsfunktion die Funktion $f$ mit
        $$ f(x) = \left\{
        \begin{array}{ll}
        0 & x \leq 0 \\
        1 & \, \textrm{sonst} \\
...
def _(mo):
    mo.md(
        ...
        <details>
        <summary><b>🗬 Trainings- und Testdaten</b></summary>
        ...
        ---
        ✎ Erstelle eine Liste mit allen Datenpunkten als ($x_1$, $x_2$, $l$)-Tripel. $l$ bezeichnet dabei das <b>Label</b> des Datenpunkts. Gehört der Datenpunkt zu der Klasse der Blaumeiseneier, so ist $l=0$, ansonsten ist $l=1$. Durchmische die Liste und unterteile die Daten in Trainings- und Testdaten in einem Verhältnis von 8:2.
        """
    )
    return
...
def _(mo):
    mo.md(
        r"""
        ✎ Zähle wie viele Punkte das Perzeptron mit deiner Wahl der Gewichte $w_1$ und $w_2$ und des Bias $b$ richtig klassifiziert und gib die Genauigkeit in Prozent aus. Benutze dabei das von dir implementierte Perzeptron aus der ersten Aufgabe.
        """
    )
    return
...
def _(mo):
    mo.md(
        r"""
        🗬 *In unserem Datensatz gibt es gleich viele Blaumeisen- und Greifvogel-Eier. Wenn also jemand rät und sich jeweils zufällig für eine Klasse entscheidet, liegt seine Rategenauigkeit bei 50%. Wenn dein Perzeptron eine ähnliche Genauigkeit aufweist, dann hat es definitiv noch Verbesserungspotential.*

        <details>
        <summary><b>🗬 Geometrische Betrachtung</b></summary>
        ...
        ---
        ✎ Setze folgende Formeln zur Gewichtsanpassung als Funktion in Code um.

        $$w_i^{\text{neu}} = w_i^{\text{alt}} + 0.1 \cdot (o - l) \cdot x_i$$

        $$b^{\text{neu}} = b^{\text{alt}} + 0.1 \cdot (o - l)$$

        🗬 *Mit $o$ wird der Output des Perzeptrons und mit $l$ das tatsächliche Label des Datenpunkts bezeichnet. Die <b>Lernrate</b> ist in der Formel auf 0.1 gesetzt. Sie gibt an, wie stark sich der Gewichtsvektor verändern soll.*
        """
    )
    return
...
def _(mo):
    mo.md(
        ...
        ---
        ✎ Du hast oben bereits einen Datensatz erzeugt. Trainiere das Perzeptron solange, bis es jeden Punkt richtig klassifiziert (maximal aber drei Epochen lang).

        ✎ *Zusatzaufgabe:* Zeichne nach jeder Änderung des Gewichtsvektors $\vec{w}$ ein Schaubild mit den Punkten und der Entscheidungsgerade ein. Färbe die Punkte oberhalb der Geraden mit der Farbe der Blaumeisen-Eier und die Punkte unterhalb der Geraden mit der Farbe der Greifvogel-Eier.
        """
    )
    return
...
def _(mo):
    mo.md(
        r"""
        Wenn du alles richtig gemacht hast, sollte der Algorithmus nach wenigen Schritten die Datenpunkte durch eine Gerade voneinander trennen. 

        ---
        ✎ Führe deinen Code nun mit dem unteren Datensatz noch einmal aus. Beobachte, was passiert, und passe gegebenenfalls die Lernrate an.
        """
    )
    return
...
def _(mo):
    mo.md(
        r"""
        Das Perzeptron kann Datenpunkte zweier Klassen durch eine Gerade trennen. 

        ---
        ✎ Führe das folgende Feld aus. Erkennst du ein Problem? Führe deinen implementierten Algorithmus auf diesem Datensatz aus. Was passiert?
        """
    )
    return
...
def _(mo):
    mo.md(
        ...
        🗬 *Überlege dir, in welchen Szenarien das Perzeptron an seine Grenzen stößt und klicke anschließend die Lösungen an.*
